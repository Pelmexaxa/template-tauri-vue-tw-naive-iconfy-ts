<template>
  <n-layout class="min-h-screen bg-neutral-950">
      ...
      <n-flex align="center" justify="space-between" wrap>
        <n-flex align="center" :size="12">
          ...
          <div>
            ...
            <n-text depth="3"
              >Vue · Tailwind · Naive UI · Iconify · TypeScript</n-text
            >
          </div>
        </n-flex>
        <n-tag type="success" round size="small">v0.1.0</n-tag>
        <n-button class="w-24" @click="toggleTheme">
          {{ isDark ? "Dark" : "Light" }}
        </n-button>
      </n-flex>
    </n-layout-header>
    ...
    <n-layout-content class="p-6">
                ...
                <n-thing>
                  ...
                  <template #header>
                    <n-flex align="center" :size="8">
                      <span>{{ tech.name }}</span>
                      <n-tag :type="tech.tag" size="tiny" round>{{
                        group.category
                      }}</n-tag>
                    </n-flex>
                  </template>
      ...
      <n-card title="Rust ↔ Vue => Calc" size="small" class="max-w-xl mx-auto">
        <n-space vertical :size="12">
          <n-input-number
            v-model:value="numbers.number1"
            placeholder="Введите число"
            clearable
          />
          <n-input-number
            v-model:value="numbers.number2"
            placeholder="Введите число"
            clearable
          />
          <n-button type="primary" block @click="calc">
            Деление и ошибки
          </n-button>
          <n-text v-if="calcMsg" type="success">{{ calcMsg }}</n-text>
        </n-space>
      </n-card>
    </n-layout-content>
  </n-layout>
...
import { useTheme } from "@/scripts/stores/theme";
...
const techStack: TechGroup[] = [
  {
    ...
    items: [
      {
        name: "Vue 3",
        desc: "Composition API, script setup",
        icon: "icon-[logos--vue]",
        tag: "success",
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      {
        name: "Naive UI",
        desc: "Vue 3 компоненты",
        icon: "icon-[logos--naiveui]",
        tag: "info",
      },
      {
        name: "Iconify",
        desc: "Иконки через Tailwind",
  ...
  },
  {
    ...
    items: [
      {
        name: "Tauri 2",
        desc: "Нативное приложение",
        icon: "icon-[logos--tauri]",
        tag: "success",
      },
      {
        name: "Rust",
        desc: "Backend-команды",
        icon: "icon-[logos--rust]",
        tag: "warning",
      },
      {
        name: "Serde",
        desc: "Сериализация данных",
...
async function calc() {
  calcMsg.value = await invoke("calc", {
